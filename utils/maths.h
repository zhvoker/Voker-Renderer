#ifndef RENDERERS_MATHS_H
#define RENDERERS_MATHS_H
#include <cmath>
#include <vector>
#include <cassert>
#include <iostream>
...
template<size_t DIM,typename T>vec<DIM,T> operator+=(vec<DIM,T>& lhs, const vec<DIM,T>& rhs) {
    for (size_t i=DIM; i--; lhs[i]=lhs[i]+rhs[i]);
    return lhs;
}
...
template<size_t LEN,size_t DIM, typename T> vec<LEN,T> proj(const vec<DIM,T> &v) {
    vec<LEN,T> ret;
    for (size_t i=LEN; i--; ret[i]=v[i]); //     int k = 4; std::cout<<k--; // 输出为4 注意i--为表达式 执行完成后再使用i的值才会减
    return ret;
}
...
float Clamp(float x, float min, float max); // 模板函数声明 实现见.cpp文件
Vec3f Mix(Vec3f a, Vec3f b, float weight);
float Length(Vec3f a);
Vec3f Vecmul(Vec3f a, Vec3f b); // 区分点乘 返回三个分量
#endif //RENDERERS_MATHS_H
